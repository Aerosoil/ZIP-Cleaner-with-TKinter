...
WORDS = ["CAG", "Changi", "airport", "CT05", "Joestar", "Brando", "Kujo",
...
def clean_data_file(file_path):
    try:
        ...
        for word in WORDS:
            content = re.sub(re.escape(word), "[REDACTED][ZA][WARUDO][KONO][DIO][DA]", content, flags=re.IGNORECASE)
...
def process_zip(zip_path, password):
    ...
    out_zip = os.path.join(
        OUT_F, os.path.splitext(filename)[0] + "_cleaned.zip"
    )
...
def clean_all_zips(folder, password):
    ...
    for i, file in enumerate(zips, 1):
        ...
        ok = process_zip(os.path.join(folder, file), password)
        if ok:
...
for word in WORDS:
    for char in word:
        password_entry = tk.Entry(root, show=f"{char}")#🗿
password_entry.pack(pady=5)

btn_clean = tk.Button(root, text="Clean Zips Files", command=start_cleaning)
